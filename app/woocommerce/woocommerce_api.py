...
if not all([WC_BASE_URL, WC_API_KEY, WC_API_SECRET]):
    raise RuntimeError("Missing WooCommerce API credentials in .env (WC_BASE_URL / WC_API_KEY / WC_API_SECRET)")
...
if (WP_MEDIA_USER or WP_MEDIA_APP_PASS) and not (WP_MEDIA_USER and WP_MEDIA_APP_PASS):
    logger.warning("Only one of WP_MEDIA_USER / WP_MEDIA_APP_PASS supplied. Media uploads will fail.")
...
async def _wp_media_client():
    if not (WP_MEDIA_USER and WP_MEDIA_APP_PASS):
        raise RuntimeError("WP_MEDIA_USER / WP_MEDIA_APP_PASS missing. Cannot upload media.")
...
async def wc_search_by_sku(sku: str, statuses: Optional[Sequence[str]] = None) -> List[Dict[str, Any]]:
    """
...
async def wc_regenerate_lookup_table() -> bool:
    ...
    Never raise.
    """
    new_url = f"{WC_BASE_URL}/wp-json/wc/v3/system_status/tools/regenerate_product_lookup_table"
    ...
    async with await _wc_client() as c:
        ...
        try:
            ...
            logger.warning("Woo lookup-table regen not available: %s", e_old)
        except Exception as e:
...
async def wc_update_product_images(product_id: int, images_payload: List[Dict[str, Any]]) -> Dict:
    return await wc_put(f"products/{product_id}", {"images": images_payload})

async def wp_upload_media(binary: bytes, filename: str, mime_type: str) -> int:
    """
    Upload to WP media library via WP REST (/wp/v2/media).
    Requires WP media auth (application password or basic auth capable user).
    """
    if not (WP_MEDIA_USER and WP_MEDIA_APP_PASS):
        raise RuntimeError("WP_MEDIA_USER / WP_MEDIA_APP_PASS not set; cannot upload media.")
